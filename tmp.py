...
def generate_local_text(
    ...
    system_prompt = (
        "You are a privacy-preserving query rewriter for a cloud LLM.\n"
        ...
        "- KEEP only the structure of the financial situation that is needed to answer.\n"
        "- Output ONLY the rewritten, anonymized question. No preface, no bullets, no analysis.\n"
        "- Do NOT add new personal details or numbers.\n"
    )
    messages = [
    ...
    final_prompt: str = tokenizer.apply_chat_template(
        messages,
        tokenize=False,
        add_generation_prompt=True,  # assistant 답변 자리까지
    )
...
def parse_args() -> argparse.Namespace:
    ...
    # local generation
    p.add_argument("--local_max_new_tokens", type=int, default=256)
...
def main() -> None:
    ...
    print(f"Read {len(rows)} lines from {args.question_jsonl}")
    ...
    print("Ontology used:", json.dumps(ontology.granularities, indent=2))
    ...
    for it in items:
        ...
        """
        passthrough_input_line = build_batch_input_line(
            custom_id=f"q{qid:06d}_passthrough",
            cloud_model=args.cloud_model,
            system_prompt=args.system_prompt,
            user_content=passthrough_prompt,
        ...
        baseline_input_line = build_batch_input_line(
            custom_id=f"q{qid:06d}_baseline",
            cloud_model=args.cloud_model,
            system_prompt=args.system_prompt,
            user_content=baseline_prompt,
        ...
        logit_input_line = build_batch_input_line(
            custom_id=f"q{qid:06d}_logit",
            cloud_model=args.cloud_model,
            system_prompt=args.system_prompt,
            user_content=logit_prompt,
        ...
        logging.info(f"Processed qid={qid} (line={it['source_line']})")
        logging.info(f"Passthrough: {passthrough_prompt}")
        logging.info(f"Baseline: {baseline_prompt}")
        logging.info(f"Logit: {logit_prompt}")

        # temporary save
        with open(args.out_jsonl, 'a', encoding='utf-8') as f:
            f.write(json.dumps(passthrough_input_line, ensure_ascii=False) + "\n")
            f.write(json.dumps(baseline_input_line, ensure_ascii=False) + "\n")
            f.write(json.dumps(logit_input_line, ensure_ascii=False) + "\n")
